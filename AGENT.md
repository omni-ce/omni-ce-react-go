...
3. **Module Structure Rules**:
   - **Model (`./core/modules/[nama_module]/model/[nama_module].model.go`)**: Create a GORM struct with appropriate fields (including tracking fields like `CreatedAt`, `CreatedBy`, `UpdatedAt`, `UpdatedBy`), a `Map()` method, and a `Seed() []ModelName` method.
     ...
     - **Address Fields Rules**: Any GORM model that supports a physical/delivery address MUST define exactly **two** address fields: `Address string json:"address" gorm:"type:text;not null"` and `AddressCode string json:"address_code" gorm:"type:varchar(255);not null"`.
   - **Handler (`./core/modules/[nama_module]/[nama_module].handler.go`)**: Implement the CRUD and pagination logic, retrieving user data via `function.JwtGetUser(c)` for audit logs.
     - **Address Handling in Paginate**: For GORM models containing address fields, the backend handler's `Paginate` function MUST resolve `AddressCode` to a readable full address string by importing `"react-go/core/function/location"` and calling `location.GetFull(address_code)` for each entity in the results, returning the full string in the JSON response as `full_address`. See [company_entity.handler.go](file:///Users/jefripunza/Documents/Projects/react-go/core/modules/company/company_entity.handler.go) or [supplier_entity.handler.go](file:///Users/jefripunza/Documents/Projects/react-go/core/modules/supplier/supplier_entity.handler.go) for implementation.
...
- **Automated Unit Testing**: If the FE and BE for a module are already created, but there are no unit tests under `./tests/modules/**`, you MUST create them: write 1 playwright E2E unit test per page/entity, group them in an `index.ts` within the module's subfolder under `./tests/modules/`, and import/call it in [start.spec.ts](file:///Users/jefripunza/Documents/Projects/react-go/tests/start.spec.ts). When creating or editing a test file, ALWAYS extract the record creation/insertion steps into a dedicated, parameter-driven reusable helper function (e.g. `insertNewUser`, `insertNewCompanyEntity`, `inputNewVariant`) at the bottom of the file to maximize modularity and reusability.
